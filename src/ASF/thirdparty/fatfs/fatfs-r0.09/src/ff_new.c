...
/                   Fixed null pointer dereference on attempting to delete the root directory.
/ Feb 09,'15 R0.11  Added f_findfirst(), f_findnext() and f_findclose(). (_USE_FIND)
/                   Fixed f_unlink() does not remove cluster chain of the file. (appeared at R0.10c)
/                   Fixed _FS_NORTC option does not work properly. (appeared at R0.10c)
/ Mar 09, '15       Fixed At the thread-safe configuration (_FS_REENTRANT == 1), an unexpected operation to the volume can result a wrong volume lock.
/---------------------------------------------------------------------------*/
...
#endif
#if _MAX_SS == _MIN_SS
#define	SS(fs)	((uint16_t)_MAX_SS)	/* Fixed sector size */
#else
...
#if _STRING_H_ != 1     /* We have included the string functions from avr-libc */
/* Copy memory to memory */
static void mem_cpy (void* dst, const void* src, uint16_t cnt) {
...
/* Fill memory */
static void mem_set (void* dst, int16_t val, uint16_t cnt) {
	uint8_t *d = (uint8_t*)dst;
...
static int16_t mem_cmp (const void* dst, const void* src, uint16_t cnt) {
...
/* Check if 16 bit character is contained in the string */

static int16_t chk_chr (const char* str, int16_t chr) __attribute__ ((flatten));

static int16_t chk_chr (const char* str, int16_t chr) {
...
int pattern_matching (	/* Return value: 0:mismatched, 1:matched */
	...
	int16_t skip,			/* Number of pre-skip chars (number of ?s) */
	int16_t inf				/* Infinite search (* specified) */
)
{
	const TCHAR *pp, *np;
	uint16_t pc, nc;
...
)
{
	...
	if (!fil || !fil->fs || !fil->fs->fs_type || fil->fs->id != fil->id || (disk_status(fil->fs->drv) & ( STA_NOINIT | STA_NODISK) ))
...
FRESULT f_mkfs (
	...
	uint16_t au			/* Size of allocation unit in unit of byte or sector */
)
{
	...
	if (stat & STA_NODISK) return FR_NO_DISK;
